    ...
    useEffect(() => {
        ...
        svg.append('g')
            ...
            .on('mousemove', function(event) {
                ...
                const left = Math.min(Math.max(8, event.clientX - rect.left + 12), rect.width - 150);
                const top = Math.min(Math.max(8, event.clientY - rect.top + 12), rect.height - 80);
        ...
        <div>
            ...
            {stats && (
                <div style={{ maxWidth: 960, margin: '14px auto 0', padding: 12 }}>
                    {/* MODIFICA: Uso di CSS GRID invece di Flexbox per uniformità */}
                    <div style={{ 
                        display: 'grid', 
                        ...
                        {[1,2,3,4,5].map(p => (
                            <div key={p} style={{ 
                                /* Card styling: flex verticale per allineare il contenuto */
                                display: 'flex',
                                flexDirection: 'column',
                                justifyContent: 'space-between',
                                //background: 'rgba(255, 255, 255, 0)', // Opzionale: sfondo leggero
                                borderRadius: '6px',
                                padding: '8px'
                            }}>
                                <div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <div style={{ width: 12, height: 12, background: colorScale(p) || '#999', border: '2px solid #333', borderRadius: 2 }} />
                                        <div style={{ fontWeight: 'normal' }}>{phaseLabels[p] || `P${p}`}</div>
                                    </div>
                                    <div style={{ fontSize: 13, color: '#333', marginTop: 6 }}>
                                        Area: {stats.areaPercent[p].toFixed(1)}% <br/>
                                        <span style={{opacity: 0.7}}>({stats.counts[p]}/{stats.totalAreas})</span>
                                    </div>
                                    <div style={{ fontSize: 13, color: '#333', marginTop: 4 }}>
                                        Pop: {stats.popPercent[p].toFixed(1)}% <br/>
                                        <span style={{opacity: 0.7}}>({formatShort(Math.round(stats.popByPhase[p]))})</span>
                                    </div>
                                </div>
                                {/* Barra di progresso spinta verso il basso */}
                                <div style={{ height: 8, background: '#eef0f3', borderRadius: 6, marginTop: 10, overflow: 'hidden' }}>
